...
export default function VoiceChannel({ channelId, channelName, onClose }: VoiceChannelProps) {
  ...
  const createPeerConnection = useCallback((remoteSocketId: string, isInitiator: boolean) => {
    // BUG 3 FIX: Guard against double creation — if a PC already exists, return it
  ...
  const joinVoice = useCallback(async () => {
    ...
    try {
      ...
      socket.on("voice-offer", async ({ from, offer }: { from: string; offer: RTCSessionDescriptionInit }) => {
        ...
        // BUG 2 FIX: flush any ICE candidates that arrived before setRemoteDescription
        const buffered = iceCandidateBufferRef.current.get(from) ?? [];
        for (const candidate of buffered) {
          try { await pc.addIceCandidate(new RTCIceCandidate(candidate)); } catch { /* ignore stale */ }
        }
      ...
      socket.on("voice-answer", async ({ from, answer }: { from: string; answer: RTCSessionDescriptionInit }) => {
        ...
        if (pc) {
          ...
          // flush buffered ICE candidates for the answerer side too
          const buffered = iceCandidateBufferRef.current.get(from) ?? [];
          for (const candidate of buffered) {
      ...
      socket.on("ice-candidate", async ({ from, candidate }: { from: string; candidate: RTCIceCandidateInit }) => {
        ...
        // BUG 2 FIX: if remote description not yet set, buffer the candidate
