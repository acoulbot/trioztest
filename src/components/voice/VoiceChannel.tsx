...
import { patchedOffer, patchedAnswer, patchOpusSDP } from "@/lib/sdpUtils";
...
export default function VoiceChannel({ channelId, channelName, onClose }: VoiceChannelProps) {
  ...
  /* ── Sound effects ──────────────────────────────────────────────────── */
  useEffect(() => {
    connectionSfxRef.current    = Object.assign(new Audio("/sounds/connection.mp3"),    { preload: "auto" });
    disconnectionSfxRef.current = Object.assign(new Audio("/sounds/disconnection.mp3"), { preload: "auto" });
  ...
  const playSound = useCallback((ref: React.RefObject<HTMLAudioElement | null>) => {
    ...
    try { const c = ref.current.cloneNode() as HTMLAudioElement; c.volume = 0.5; c.play().catch(() => {}); } catch {}
  }, []);
  ...
  const createPeerConnection = useCallback((remoteSocketId: string, isInitiator: boolean) => {
    ...
    if (isInitiator) {
      // Use patched offer (Opus FEC + bitrate)
  ...
  const startSpeakingDetection = useCallback(() => {
    // Use the RAW mic stream for speaking detection so the analyser
    // always sees the real signal regardless of bypass state.
  ...
  const joinVoice = useCallback(async () => {
    ...
    try {
      ...
      socket.on("user-muted",    ({ socketId, muted }: any) =>
        setUsers(p => p.map(u => u.socketId === socketId ? { ...u, muted } : u))
      );
      socket.on("user-speaking", ({ socketId, speaking }: any) =>
...
function NsButtonContent({ status, enabled }: { status: NSStatus; enabled: boolean }) {
  if (status === "loading") {
    return (
...
function NsIcon({ className = "w-5 h-5" }) {
  // Waveform with a "filter" dot — custom icon for noise suppressor
  return (
    <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
